...
function xblog_plus_new_optins_texts() {
    ...
    if(get_option('xpntex32')){
        return;
    }
	$class = 'eye-notice notice notice-warning is-dismissible';
	$message = __( '<strong> Hi Buddy!! Now You are usign Free version of the X Blog theme. <span style="font-weight:700">Upgrade to XBlog PRO and unlock advanced features</span> <br> By upgrading to XBlog PRO you get access to advanced PRO features, unlited options and much more.<span style="font-weight:700"> So upgrade PRO Today!!</span></strong>', 'x-blog-plus' );
...
function xblog_plus_new_optins_texts_init(){
    if(isset($_GET['xpnotice']) && $_GET['xpnotice'] == 1 ){
        delete_option( 'xpntex31');
        update_option( 'xpntex32', 1);
    }
}
